        ...
        <div class="col-md-12">
            <div class="card">
                <div class="table-responsive">
                    <table id="domains" class="table table-hover dataTable">
                        <!-- Placeholder for DataTables content -->
                         ...
                         <tbody>
                            <?php foreach($domainsData as $domain){ ?>
                                <tr>
                                <td><?=$domain["id"]?></td>
                                <td><?=$domain["domain"]?></td>
                                <td><?=$domain["company"]?></td>
                                <td><?=$domain["location"]?></td>
                                <td><?=$domain["owned_by"]?></td>
                                <td><?=$domain["hosted_by"]?></td>
                                <td><?=$domain["domain_ext"]?></td>
                                <td><?=$domain["domain_cost"]?></td>
                                <td><?=$domain["domain_r"]?></td>
                                <td><?=$domain["host_pkg_gb"]?></td>
                                <td><?=$domain["hosting_r"]?></td>
                                <td><?=$domain["domain_notes"]?></td>
                                <td><?=$domain["last_edited_user"]?></td>
                                </tr>
                            <?php } ?>
                         </tbody>
                    </table>
...
<script>
    var domains = <?php echo json_encode($domainsData); ?>
    console.log(domains);
    ...
    $(document).ready(function () {
        ...
        let domainsTable = new DataTable('#domains', {
            fixedColumns: true,
            paging: true,
            searching: true,
            // scrollCollapse: true,
            ...
            columns: [
                ...
                { data: 'last_edited_user', title: 'Last Edited By' }
            ],
            data: domains
        ...
        $('#search').keyup(function () {
            domainsTable.search($(this).val()).draw();
        });

        // Handle row click event
        $('#domains tbody').on('click', 'tr', function () {
            let itemData = domainsTable.row(this).data();
            $.ajax({
                url: '/domains/loadForm',
                type: 'POST',
                data: itemData,
